"""Garment segmentation using briaai/RMBG-1.4 via HuggingFace transformers.

Produces a transparent-PNG output with the garment isolated on an alpha channel.
The mask is resized to match the original image dimensions when the model
returns a different resolution.

Swap to a different model by changing MODEL_NAME or implementing a new
``BaseSegmenter`` subclass — the pipeline only depends on the abstract
interface.
"""
...
class RMBGSegmenter(BaseSegmenter):
    """Segmenter using briaai/RMBG-1.4 from HuggingFace."""
    ...
    # ------------------------------------------------------------------
    # Internal steps (each handles one concern)
    # ------------------------------------------------------------------
    ...
    def _pick_best_mask(self, outputs: list[dict]) -> Image.Image:
        """From a list of ``{"label": …, "score": …, "mask": …}`` dicts,
    ...
    def _validate_mask_size(self, mask: Image.Image) -> None:
        """Reject masks that are so small they'd produce a degraded result.

        Called *before* resize so the check uses the model's raw output
        resolution.
        """
    ...
    def _normalize_mask(self, mask: Image.Image, target: tuple[int, int]) -> Image.Image:
        """Resize mask to *target* if dimensions don't match."""
        if mask.size != target:
            mask = mask.resize(target, Image.BILINEAR)
        return mask

    def _apply_alpha(self, image: Image.Image, mask: Image.Image) -> Image.Image:
        """Apply the mask as the alpha channel of an RGBA copy of *image*."""
        rgba = image.copy()
        try:
            rgba.putalpha(mask)
